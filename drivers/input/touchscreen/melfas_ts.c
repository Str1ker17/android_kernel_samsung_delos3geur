...
#if defined(CONFIG_MACH_ARUBASLIM_OPEN) // temp. || defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN)
...
#if defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN)
	// delos model,  If read num >30, this is abnormal irq. so ignored.
	if((read_num > TS_READ_REGS_LEN)|| !(read_num%6==0) ){
		printk(KERN_ERR "[TSP ]%s. irq ignored, length error:%d,%d \n", __func__, read_num, (read_num%6));
		return;
	}
#else
	// normal case (ESD case).	
...
#if defined(CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_DELOS_OPEN)

...
#if defined(BOOT_CHECK_CRC)
static int Check_CRC(struct i2c_client *_client)
{
	unsigned char write_buf[10];
	unsigned char read_buf_array[10];
	unsigned char Pre_calc[4];
	unsigned char Calc[4];
	int result = 0;
	int i;

	printk("[TSP] %s : %d\n", __func__,__LINE__);
	//Start Pre Cals CRC read
	write_buf[0] = 0xA0; //RMI_ADDR_UNIV_CMD;
	write_buf[1] = 0x02; //UNIVCMD_GET_PRECALC_CRC32;
	if (!mms100_i2c_write(_client, write_buf, 2)) //pre calc CRC mode write
		goto ERROR_HANDLE;
	write_buf[0] = 0xAE; //RMI_ADDR_UNIV_CMD_RESULT_LENGTH;
	if (!mms100_i2c_write(_client, write_buf, 1))
		goto ERROR_HANDLE;

	mms100_i2c_read(_client, 0xAE, 1, read_buf_array);
	if (!read_buf_array[0]) //read length receive
		goto ERROR_HANDLE;

	write_buf[0] = 0xAF; //RMI_ADDR_UNIV_CMD_RESULT;
	if (!mms100_i2c_write(_client, write_buf, 1))
		goto ERROR_HANDLE;

	
	mms100_i2c_read(_client, 0xAF, read_buf_array[0], read_buf_array);
	if (!read_buf_array[0]) //pre Calc CRC receive
		goto ERROR_HANDLE;

	for (i = 0; i < 4; i++){
		Pre_calc[i] = read_buf_array[i];
		}
	
	// Pre Calc CRC read End
	pr_err("[TSP] %s : Precalc CRC32\n",__func__);
	pr_err("[TSP] %s : HEX : 0x%08X\n",__func__, (*(uint32_t*) Pre_calc));

#if 1  // If happend enter test mode fail, retry 100 time.
	i = 0;
	do //wait INT low before CRC check
	{
		write_buf[0] = 0xA0; //RMI_ADDR_UNIV_CMD;
		write_buf[1] = 0x40; //UNIVCMD_ENTER_TEST_MODE;
		if (!mms100_i2c_write(_client, write_buf, 2))
			goto ERROR_HANDLE;

		while (gpio_get_value(GPIO_TOUCH_INT))
			usleep(100);
		
		write_buf[0] = 0x0F; //RMI_ADDR_TOUCH_EVENT_PACKET_LENGTH;
		if (!mms100_i2c_write(_client, write_buf, 1))
			goto ERROR_HANDLE;
		mms100_i2c_read(_client, 0x0F, 1, read_buf_array);
		if (!read_buf_array[0])
			goto ERROR_HANDLE;
		pr_err("[TSP] %s : >> RMI_ADDR_TOUCH_EVENT_PACKET_LENGTH : %d\n",__func__, read_buf_array[0]);
		write_buf[0] = 0x10; //RMI_ADDR_TOUCH_EVENT;
		if (!mms100_i2c_write(_client, write_buf, 1))
			goto ERROR_HANDLE;
		mms100_i2c_read(_client, 0x10, 1, read_buf_array);
		if (!read_buf_array[0])
			goto ERROR_HANDLE;
		pr_err("[TSP] %s : >> RMI_ADDR_TOUCH_EVENT : 0x%02X\n",__func__, read_buf_array[0]);


		if(read_buf_array[0] != 0x0C){	
			
			i++;
			usleep(100);
			if(i>100){
				pr_err("[TSP] %s : enter test mode, over 100\n",__func__);
				goto ERROR_HANDLE;
			}
		}

		
	} while (read_buf_array[0] != 0x0C/*RMI_UNIVCMD_MARKER*/);
	...
	write_buf[0] = 0xA0; //RMI_ADDR_UNIV_CMD;
	write_buf[1] = 0x4E; //UNIVCMD_CALC_CRC32;
	pr_err("[TSP] %s : Full Binary CRC Check\n",__func__);
	if (!mms100_i2c_write(_client, write_buf, 2))
		goto ERROR_HANDLE;
	while (gpio_get_value(GPIO_TOUCH_INT)) //CRC check End after INTR clear
		udelay(100);
	write_buf[0] = 0xAE; //RMI_ADDR_UNIV_CMD_RESULT_LENGTH;
	if (!mms100_i2c_write(_client, write_buf, 1))
	{
	...
	pr_err("[TSP] %s : read size : %d byte\n",__func__, read_buf_array[0]);
	write_buf[0] = 0xAF; //RMI_ADDR_UNIV_CMD_RESULT;
	if (!mms100_i2c_write(_client, write_buf, 1))
		goto ERROR_HANDLE;

	mms100_i2c_read(_client, 0xAF, read_buf_array[0], read_buf_array);
	if (!read_buf_array[0]) //Calc CRC receive
		goto ERROR_HANDLE;

	for (i = 0; i < 4; i++){
		Calc[i] = read_buf_array[i];
	}

	pr_err("[TSP] %s : HEX : 0x%08X\n",__func__, (*(uint32_t*) Calc));
	write_buf[0] = 0xA0; //RMI_ADDR_UNIV_CMD;
	write_buf[1] = 0x4F; //UNIVCMD_EXIT_TEST_MODE;
	if (!mms100_i2c_write(_client, write_buf, 2))
		goto ERROR_HANDLE;
	...
	if (*(uint32_t*)Pre_calc == *(uint32_t*)Calc)
		result = 1;

	pr_err("[TSP] %s : CRR Check result : %d\n",__func__, result);

	return result;

	ERROR_HANDLE: pr_err ("[TSP] Check_CRC : i2c error \n");
		return -1;
}
#endif
...
#if defined(BOOT_CHECK_CRC)
	if(mbin_info[SEC_CONFIG].version == ts_info[SEC_CONFIG].version){
		if((Check_CRC(_client))==0){
			printk( "[TSP] %s Check_CRC : Fail. Config Update\n", __func__);
			section_update_flag[SEC_CONFIG] = true;
		}
	}
#endif
...
{   
    ...
    pr_info("[TSP ISC] %s\n", __func__);
#if defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN)
	ret_read = mms100_i2c_read(_client, ISC_ADDR_VERSION, SECTION_NUM, rd_buf); 

	if (ret_read < 0)
	{
	pr_info("[TSP ISC] %s,%d: i2c read fail[%d] \n", __FUNCTION__, __LINE__, ret_read);
		return ISC_I2C_ERROR;
	}
	pr_info("\tTS : Section(%d) CORE version: 0x%02X\n", SEC_CORE, rd_buf[SEC_CORE]);
#endif
	...
	#if defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN)
		if(rd_buf[SEC_CORE]==(0x51))
			ret += request_firmware(&(fw_mbin[1]),"tsp_melfas/delos_g2/CORE.fw", &_client->dev);
		else
	#endif		
	...
	#if defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN)
		if(rd_buf[SEC_CORE]==(0x51))
			ret += request_firmware(&(fw_mbin[2]),"tsp_melfas/delos_g2/CONF.fw", &_client->dev);
		else
	#endif

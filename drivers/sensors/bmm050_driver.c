		...
		struct device_attribute *attr, char *buf)
{
	...
	if( (client_data->value.datax == 0) && (client_data->value.datay == 0) )
	{
		client_data->value.datax = prev_value.datax;
		client_data->value.datay = prev_value.datay;
		client_data->value.dataz = prev_value.dataz;
	}
	else
	{
		prev_value.datax = client_data->value.datax;
		prev_value.datay = client_data->value.datay;
		prev_value.dataz = client_data->value.dataz;
	}

	if (client_data->value.datax == BMM050_OVERFLOW_OUTPUT_S32)
		client_data->value.datax = BMM050_OVERFLOW_OUTPUT_S32_XY;
	if (client_data->value.datay == BMM050_OVERFLOW_OUTPUT_S32)
		client_data->value.datay = BMM050_OVERFLOW_OUTPUT_S32_XY;
	if (client_data->value.dataz == BMM050_OVERFLOW_OUTPUT_S32)
		client_data->value.dataz = BMM050_OVERFLOW_OUTPUT_S32_Z;
